...
```text
java-teaching-assistant/src/main/java/com/javaProgram/
├── actions/                                    # 用户操作类（右键菜单、快捷操作）
│   ├── AddToContextAction.java                # 添加选中代码到AI上下文
│   ├── AskQuestionAboutCodeAction.java        # 针对选中代码提问
│   ├── ClearContextAction.java                # 清空上下文
│   └── ModifyCodeAction.java                  # 请求AI修改代码
│
├── services/                                   # 服务层（核心业务逻辑）
│   ├── AiServiceClient.java                   # AI服务HTTP客户端（SSE流式、代码修改、意图识别）
│   ├── CodeDiffResult.java                    # 代码差异结果数据类
│   ├── ContextService.java                    # 上下文管理服务
│   ├── HttpRequestConfig.java                 # HTTP请求配置类
│   ├── PendingModificationManager.java        # 待确认修改管理器
│   └── RequestQueueManager.java               # 请求队列管理器（最多3个，串行处理）⭐
│
├── ui/                                         # 用户界面
│   ├── ChatToolWindowContent.java              # 聊天窗口主控制器（协调UI组件、请求队列、意图识别）
│   ├── ChatToolWindowFactory.java              # 工具窗口工厂
│   ├── IntelliJDiffViewer.java                 # 代码差异查看器（使用IntelliJ内置Diff工具）
│   │
│   ├── components/                            # UI组件包
│   │   ├── ChatInputPanel.java                # 聊天输入面板（多行输入、自动高度）
│   │   ├── ChatMessagePanel.java              # 聊天消息显示面板（滚动、自动滚动）
│   │   ├── ContextDisplayPanel.java           # 上下文显示面板（显示、跳转、删除）
│   │   ├── MessageBubbleFactory.java          # 消息气泡工厂（用户/AI气泡、圆角布局）
│   │   ├── ModificationConfirmationPanel.java # 代码修改确认面板（统一界面、接受/拒绝）
│   │   ├── QueueDisplayPanel.java             # 请求队列显示面板（实时状态、图标、预览）⭐
│   │   ├── ThinkingIndicatorManager.java      # AI思考指示器管理器（动画、自动显示/隐藏）
│   │   │
│   │   └── handlers/                          # UI组件事件处理器
│   │       ├── FileDropHandler.java          # 文件拖拽处理器
│   │       └── InputHintManager.java          # 输入提示管理器
│   │
│   ├── handlers/                              # 事件处理器包
│   │   └── AiResponseHandler.java             # AI响应处理器（SSE流式、逐块追加、状态管理）
│   │
│   └── layout/                                # 自定义布局包
│       └── WrapLayout.java                    # 自动换行布局管理器（用于上下文标签）
│
└── utils/                                      # 工具类包
    ├── CodeNavigationUtil.java                # 代码导航工具（跳转编辑器、定位行号、可点击效果）
    ├── FileContentReader.java                 # 文件内容读取工具
    ├── HttpUtil.java                          # HTTP工具类（统一请求、SSE流式、错误处理）
    ├── JsonUtil.java                          # JSON工具类（简单解析、转义、提取、构建）
    └── MarkdownToHtml.java                    # Markdown转HTML工具
```
...
| 文件名                               | 职责               | 关键功能                                                                                                                                                |
| ------------------------------------ | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ChatInputPanel.java`                | 聊天输入面板       | - 多行文本输入（自动高度调整）<br>- 发送按钮<br>- **清空输入框方法**<br>- Enter发送，Shift+Enter换行                                                    |
...
```text
ai-code-helper/src/main/java/com/example/aicodehelper/
├── AiCodeHelperApplication.java                # Spring Boot 启动类（应用程序入口）
│
├── ai/                                         # AI核心模块
│   ├── AiCodeHelperService.java               # AI服务接口（聊天、流式、RAG、代码修改、意图识别）
│   ├── AiCodeHelperServiceFactory.java        # AI服务工厂（Langchain4j构建、工具注册、RAG配置）
│   │
│   ├── guardrail/                             # 输入安全控制
│   │   └── SafeInputGuardrail.java            # 输入安全检查（敏感词、注入攻击、内容过滤）
│   │
│   ├── listener/                               # 模型监听器
│   │   └── ChatModelListenerConfig.java       # 聊天模型监听器配置（请求/响应监听、Token统计、性能监控）
│   │
│   ├── model/                                 # AI模型配置
│   │   └── QwenChatModelConfig.java           # 通义千问模型配置（API密钥、参数、Bean创建）
│   │
│   ├── rag/                                   # 检索增强生成
│   │   ├── EnhancedDocumentLoader.java        # 文档加载器（PDF加载、文档分块、向量生成）
│   │   └── RagConfig.java                     # RAG配置类（嵌入模型、存储、检索器、文档变更检测）⭐
│   │
│   └── tools/                                 # AI工具类（Function Calling）
│       └── FileReaderTool.java                # 文件读取工具（列出目录、读取文件、搜索代码）⭐
│
├── config/                                     # 配置类
│   ├── AiModelConfig.java                     # AI模型配置（嵌入模型配置）
│   └── CorsConfig.java                        # 跨域配置（允许跨域、源/方法/头配置）
│
├── controller/                                 # 控制器层
│   └── AiController.java                      # AI接口控制器（聊天、RAG、代码修改、意图识别、项目路径）
│
├── dto/                                        # 数据传输对象
│   └── CodeDiffResult.java                    # 代码差异结果（原始/修改后代码、差异块、JSON序列化）
│
└── util/                                       # 工具类
    └── DiffUtils.java                         # 差异计算工具（代码差异、差异块生成、统一格式）
```
...
- UI Component Handlers: 2 个文件（FileDropHandler, InputHintManager）
- UI Handlers: 1 个文件
- UI Layout: 1 个文件
- Utils: 3 个文件
- **总计：25 个核心Java文件**
...
| **文件拖拽粘贴** ⭐ | FileDropHandler<br>InputHintManager<br>ChatInputPanel                   | -                                                          |
...
```
┌─────────────────────┐           ┌────────────────────────┐
│  IntelliJ IDEA      │           │   Linux 服务器         │
│  插件 (客户端)      │  ────────>│   Spring Boot 后端     │
│  - 无需配置         │   HTTP    │   (统一管理API Key)    │
│  - 开箱即用         │   8081    │   (systemd 服务)       │
└─────────────────────┘           └────────────────────────┘
    多个用户电脑                         集中管理服务器
```
...
后端服务部署在 Linux 服务器上（`http://111.229.81.45:8081`）
...
- 下载 `java-teaching-assistant-1.0-SNAPSHOT.zip`
