...
public class MarkdownToHtml {
    ...
    public static String convert(String markdown, Color textColor) {
        ...
        while (codeBlockMatcher.find()) {
            ...
            codeBlockPlaceholders.put(placeholder, 
                "<pre style='background-color: " + codeBlockBgHex + "; border: 1px solid " + darkenColorHex(codeBlockBgHex, 0.2) + "; border-radius: 4px; padding: 10px; margin: 8px 0; overflow-x: auto; font-family: monospace; font-size: 12px; color: " + codeTextColorHex + "; word-wrap: break-word;'><code style='color: " + codeTextColorHex + "; word-wrap: break-word;'>" + code + "</code></pre>");
        ...
        // 按从多到少的顺序处理，确保不会互相干扰
        html = html.replaceAll("(?m)^##### (.+)$", "<h5 style='margin: 10px 0 6px 0; font-size: 14px; font-weight: bold;'>$1</h5>");
        html = html.replaceAll("(?m)^#### (.+)$", "<h4 style='margin: 11px 0 7px 0; font-size: 15px; font-weight: bold;'>$1</h4>");
        html = html.replaceAll("(?m)^### (.+)$", "<h3 style='margin: 12px 0 8px 0; font-size: 16px; font-weight: bold;'>$1</h3>");
        html = html.replaceAll("(?m)^## (.+)$", "<h2 style='margin: 14px 0 10px 0; font-size: 18px; font-weight: bold;'>$1</h2>");
        html = html.replaceAll("(?m)^# (.+)$", "<h1 style='margin: 16px 0 12px 0; font-size: 20px; font-weight: bold;'>$1</h1>");
        ...
        // 5. 处理行内代码 `code`（需要在代码块之后处理，避免冲突）
        html = html.replaceAll("`([^`]+)`", "<code style='background-color: " + codeBlockBgHex + "; color: " + codeTextColorHex + "; padding: 2px 4px; border-radius: 3px; font-family: monospace; font-size: 12px;'>$1</code>");
        ...
        // 10. 处理引用 >
        html = html.replaceAll("(?m)^> (.+)$", "<blockquote style='border-left: 3px solid #ddd; padding-left: 12px; margin: 8px 0; color: #666;'>$1</blockquote>");
        
        // 11. 处理链接 [text](url)
        html = html.replaceAll("\\[([^\\]]+)\\]\\(([^\\)]+)\\)", "<a href='$2' style='color: #0066cc; text-decoration: none;'>$1</a>");
        ...
        html = "<div style='font-family: -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, \"Helvetica Neue\", Arial, sans-serif; font-size: 14px; line-height: 1.6; color: " + colorHex + "; padding: 8px;'>" 
                + html + "</div>";
    ...
    private static String processParagraphs(String html) {
        ...
        Pattern blockElementPattern = Pattern.compile("(<(?:h[1-6]|ul|ol|li|blockquote|pre)[^>]*>.*?</(?:h[1-6]|ul|ol|li|blockquote|pre)>|<(?:h[1-6]|ul|ol|blockquote|pre)[^>]*/>)", Pattern.DOTALL);
